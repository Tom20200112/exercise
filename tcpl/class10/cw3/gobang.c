...
void thinkPosition(int *x, int *y, int side) {
    ...
    /*printf("Computer's linkNum1:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][side - 1].info[0].linkNum);
        }
        printf("\n");
    }

    printf("Computer's linkNum2:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][side - 1].info[1].linkNum);
        }
        printf("\n");
    }

    printf("Computer's linkNum3:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][side - 1].info[2].linkNum);
        }
        printf("\n");
    }

    printf("Computer's linkNum4:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][side - 1].info[3].linkNum);
        }
        printf("\n");
    }

    printf("Player's linkNum1:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][2 - side].info[0].linkNum);
        }
        printf("\n");
    }

    printf("Player's linkNum2:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][2 - side].info[1].linkNum);
        }
        printf("\n");
    }

    printf("Player's linkNum3:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][2 - side].info[2].linkNum);
        }
        printf("\n");
    }

    printf("Player's linkNum4:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][2 - side].info[3].linkNum);
        }
        printf("\n");
    }

    printf("Computer's oppNum1:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][side - 1].info[0].oppNum);
        }
        printf("\n");
    }

    printf("Computer's oppNum2:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][side - 1].info[1].oppNum);
        }
        printf("\n");
    }

    printf("Computer's oppNum3:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][side - 1].info[2].oppNum);
        }
        printf("\n");
    }

    printf("Computer's oppNum4:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][side - 1].info[3].oppNum);
        }
        printf("\n");
    }

    printf("Player's oppNum1:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][2 - side].info[0].oppNum);
        }
        printf("\n");
    }

    printf("Player's oppNum2:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][2 - side].info[1].oppNum);
        }
        printf("\n");
    }

    printf("Player's oppNum3:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][2 - side].info[2].oppNum);
        }
        printf("\n");
    }

    printf("Player's oppNum4:\n");
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            printf("%4d", MyBoardScore[i][j][2 - side].info[3].oppNum);
        }
        printf("\n");
    }*/
...
bool human2HumanNextMove(int currentSide) {
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            evalueScore(i, j, currentSide);
            evalueScore(i, j, 3 - currentSide);
        }
    }
    ...
    while ((!isValid(x, y)) || forbidden(x, y, currentSide)) {
        if (!isValid(x, y)) {
...
void imagineEvalueScore(int i, int j, int side) {
    for (int m = 0; m < 4; ++m) {
...
bool sisi(int x, int y, int side) {
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            imagineEvalueScore(i, j, side);
            if (imagineWulian(i, j, side)) {
                sisiCount++;
            }
        }
    }
...
bool forbidden(int x, int y, int side) {
    return side == 1 && !wulian(x, y, side) && (changlian(x, y, side) || sansan(x, y, side) || sisi(x, y, side));
}
...
bool human2ComputerNextMove(int side, int currentSide) {
    ...
    if (side == currentSide) {
        ...
        while ((!isValid(x, y)) || forbidden(x, y, side)) {
            if (!isValid(x, y)) {
                printf("Not a valid position. Retype: ");
