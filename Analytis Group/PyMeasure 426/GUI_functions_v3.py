...
class GUI_functions:
    ...
    def __init__(self, dic_instr):
        ...
        self.instr_types = ['--None--', 'Keithley 2400', 'Keithley 2450', 'Yoko', 'SR830', 'SR860', 'iTC Teslatron',
                            'iPS Teslatron', 'Rotator', 'Janis Temperature', 'iTC 3He', 'AMI 430 3He', 'Janis Magnet', 'SIM', 'tester']
        self.dic_sweep = ['Same direction', 'Raster']
        self.dic_instr = ['None']
        self.dic_instr.extend(dic_instr)
        print(self.dic_instr)
        self.fields = ['Internal Reference', 'ch1 display', 'ch2 display', 'Frequency',
                       'Amplitude', 'Time constant', 'Increase sensitivity', 'Decrease sensitivity']
        self.K_field = ['Disable', 'Output', 'Mode',
                        'Compliance', 'Range', 'Sweep To']
        self.K2450_field = ['Disable', 'Output', 'Mode',
                            'Reading mode', 'Compliance', 'Range', 'Sweep To']
        self.Y_field = ['Disable', 'Output', 'Mode', 'Range', 'Sweep To']
        self.Loop_field = ['Instr to sweep', 'Start', 'Stop', '# Points',
                           'Wait time (s)', 'Repeat', 'Sweep direction', '# Points down']
        ...
        self.lockin_tc_val = ['10e-6', '30e-6', '100e-6', '300e-6', '1e-3', '3e-3', '10e-3', '30e-3',
                              '100e-3', '300e-3', '1', '3', '10', '30', '100', '300', '1e3', '3e3', '10e3', '30e3']
        self.keithley_comp = ['--VOLTAGE MODE--', '1.05e-6', '10.5e-6', '105e-6', '1.05e-3',
                              '10.5e-3', '105e-3', '1.05', '--CURRENT MODE--', '210e-3', '2.1', '21', '210']
        self.keithley_range = ['--VOLTAGE MODE--', '200e-3', '2', '20', '200',
                               '--CURRENT MODE--', '1e-6', '10e-6', '100e-6', '1e-3', '10e-3', '100e-3', '1']
        self.keithley2450_range = ['--VOLTAGE MODE--', '20e-3', '200e-3', '2', '20', '200',
                                   '--CURRENT MODE--', '10e-9', '100e-9', '1e-6', '10e-6', '100e-6', '1e-3', '10e-3', '100e-3', '1']
        self.yoko_range = ['--VOLTAGE MODE--', '10e-3', '100e-3', '1',
                           '10', '30', '--CURRENT MODE', '1e-3', '10e-3', '100e-3', '200e-3']
        ...
        with open('Record_folder.txt', 'rt') as f:
            for i, line in enumerate(f):
                if i != 2:
                    self.save_filename.append(
                        line.split(self.L[i])[1].replace('\n', ''))
                else:
                    self.save_filename.append(line.split(self.L[i])[1].replace(
                        '\n', '').replace('NEWLINE', '\n'))

    # Create a line with a checkbox, a colorbox to change color, a name and entry
    def ValueInstr_GUI(self, instr):
    ...
    # Create the apearence for lockins (830 and 860) moved
    def lockin_GUI(self, lockin, label, lockin_Frame):
        j = 0
        ...
        set_button.clicked.connect(lambda: set_cmd.set_lockin_param(
            reference_entry, ch1display_entry, ch2display_entry, freq_entry, amp_entry, tc_entry))
        layout = QGridLayout()
        for i, f in enumerate(self.fields):
            if i in range(6):
    ...
    # Apearence for Keithleys(2400 and 2450) moved
    def Keithely_GUI(self, instr, label, keithley_Frame, dico):
        j = 0
        ...
        out_button.setStyleSheet(
            "background-color: #008B45;" if dico[0] == True else "background-color: #CD4F39;")
        output_cmd = Commands(instr)
        out_button.clicked.connect(lambda: output_cmd.set_output(out_button))

        mode_entry = None
        mode_button = QPushButton('VOLT' if dico[1] in [
                                  'VOLT', 'voltage'] else 'CURR')
        ...
        mode_button.clicked.connect(
            lambda: mode_cmd.set_mode(mode_button, out_button))

        if instr.get_idn()['model'] == '2450':
            read_entry = None
            read_button = QPushButton('VOLT' if dico[2] in [
                                      'VOLT', 'voltage'] else 'CURR')
        ...
        comp_button.clicked.connect(
            lambda: comp_cmd.set_compliance(comp_entry))
        ...
        sweep_button.clicked.connect(
            lambda: sweep_cmd.sweep_to(sweepTime_entry, sweep_entry))
    ...
    def Yoko_GUI(self, instr, label, yoko_Frame, dico):  # Apearence for Yokos
        ...
        protec_check.toggled.connect(out_button.setDisabled)
        out_button.setStyleSheet(
            "background-color: #008B45;" if dico[0] == 'on' else "background-color: #CD4F39;")
        output_cmd = Commands(instr)
        out_button.clicked.connect(lambda: output_cmd.set_output(out_button))
    ...
    def SIM_GUI(self, instr, sim_Frame):  # Apearence for SIM
        ...
        for i in range(len(portdefault)):
            ...
            button.clicked.connect(
                lambda state, outp=output, po=port: simcmd.togglesimoutput(outp, po))
            sweep.clicked.connect(
                lambda state, st=step, va=vset, po=port: simcmd.sim_voltsweep(st, va, po))
    ...
    def RotatorLoopGUI(self, rotator, rotFrame):  # rotator GUI
        ...
        checkcapacitance.clicked.connect(
            lambda: rotcmd.checkCap(capacitancetheta, capacitancephi))
        rotcmd.checkCap(capacitancetheta, capacitancephi)
        rotcmd.checkGround(groundtheta, groundphi)

        rotatetheta.clicked.connect(lambda: rotcmd.setRotate(
            0, setTheta, frequencyTheta, amplitudeTheta, groundtheta))
        rotatephi.clicked.connect(lambda: rotcmd.setRotate(
            1, setPhi, frequencyPhi, amplitudePhi, groundphi))
        layout = QGridLayout()
    ...
    def BLoop_GUI(self, instr, label, B_Frame):
        ...
        self.start_Bramp.clicked.connect(
            lambda: B_cmd.setB(self.stop_B, self.ramp_B))
        ...
        self.check_switch.clicked.connect(
            lambda: B_cmd.checkHtrStatus(self.switch_status))
...
#        layout.addWidget(self.switch_off,5,1,1,1)
    ...
    def TLoop_GUI(self, instr, label, T_Frame):
        ...
        self.start_Tramp.clicked.connect(lambda: T_cmd.setT(
            self.stop_T, self.ramp_T, self.heatrange))
...
#        self.port_inloop.setDisabled(True)
        ...
        self.dir_inloop.currentIndexChanged.connect(
            lambda: self.disable_Line2(self.dir_inloop, self.sweepdown_inloop))
        ...
        self.instr_inloop.currentIndexChanged.connect(lambda: self.disable_Line(
            self.instr_inloop, self.start_inloop, self.stop_inloop, self.step_inloop, self.wait_inloop, self.repeat_inloop, self.sweepdown_inloop))
...
#        self.port_midloop.setDisabled(True)
        ...
        self.dir_midloop.currentIndexChanged.connect(
            lambda: self.disable_Line2(self.dir_midloop, self.sweepdown_midloop))
        ...
        self.instr_midloop.currentIndexChanged.connect(lambda: self.disable_Line(
            self.instr_midloop, self.start_midloop, self.stop_midloop, self.step_midloop, self.wait_midloop, self.repeat_midloop, self.sweepdown_midloop))

        # Adding Diagonal Plot option
        self.diagonalPlot_mid = QCheckBox('Pair instr')
        self.comboPairedInstr_mid = QComboBox()
        self.comboPairedInstr_mid.addItems(self.dic_instr)
        self.comboPairedInstr_mid.setDisabled(True)
        self.diagonalPlot_mid.toggled.connect(
            self.comboPairedInstr_mid.setEnabled)
        self.LabelFunctPair_mid = QLabel('Funtion')
        self.ComboFunctPair_mid = QLineEdit(loop_Frame)
        self.ComboFunctPair_mid.setDisabled(True)
        self.diagonalPlot_mid.toggled.connect(
            self.ComboFunctPair_mid.setEnabled)
...
#        layout.addWidget(self.port_midloop,0,3,1,1)
        ...
        # Add Diagonal Plot option
        layout.addWidget(self.diagonalPlot_mid, 10, 0, 1, 1)
        layout.addWidget(self.comboPairedInstr_mid, 10, 1, 1, 1)
        layout.addWidget(self.LabelFunctPair_mid, 11, 0, 1, 1)
        layout.addWidget(self.ComboFunctPair_mid, 11, 1, 1, 1)
...
#        self.port_outloop.setDisabled(True)
        ...
        self.dir_outloop.currentIndexChanged.connect(
            lambda: self.disable_Line2(self.dir_outloop, self.sweepdown_outloop))
        ...
        self.instr_outloop.currentIndexChanged.connect(lambda: self.disable_Line(
            self.instr_outloop, self.start_outloop, self.stop_outloop, self.step_outloop, self.wait_outloop, self.repeat_outloop, self.sweepdown_outloop, self.port_outloop))
...
#        layout.addWidget(self.port_inloop,0,3,1,1)
    ...
    # To disable line if no intrument to sweep
    def disable_Line(self, box, widget1, widget2, widget3, widget4, widget5, widget6):
...
#            self.port1=int(self.port_inloop.text())
            ...
            self.repeat2 = None
            self.repeat3 = None
            self.step1 = int(self.step_inloop.text()) if len(
                self.step_inloop.text()) != 0 else None
            self.step3 = None
            self.start1 = float(self.start_inloop.text()) if len(
                self.start_inloop.text()) != 0 else None
            self.start2 = None
            self.start3 = None
            self.stop1 = float(self.stop_inloop.text()) if len(
                self.stop_inloop.text()) != 0 else None
            self.stop2 = None
            self.stop3 = None
            ...
            self.wait2 = None
            self.wait3 = None
            self.X1_immune = np.linspace(self.start1, self.stop1, self.step1) if len(
                self.step_inloop.text()) != 0 else None
            self.X2_immune = None
            self.X3_immune = None
            self.pointdown1 = int(self.sweepdown_inloop.text()) if len(
                self.sweepdown_inloop.text()) != 0 else None
            ...
            self.X2_rev = None
            self.X3_rev = None
            self.X1_SweepDown = np.linspace(
                self.X1[-1], self.X1[0], self.pointdown1) if len(self.sweepdown_inloop.text()) != 0 else None
...
#            self.port2 = int(self.port_midloop.text())
            ...
            self.step1 = int(self.step_inloop.text()) if len(
                self.step_inloop.text()) != 0 else None
            self.step2 = int(self.step_midloop.text()) if len(
                self.step_midloop.text()) != 0 else None
            self.step3 = None
            self.start1 = float(self.start_inloop.text()) if len(
                self.start_inloop.text()) != 0 else None
            self.start2 = float(self.start_midloop.text()) if len(
                self.start_midloop.text()) != 0 else None
            self.start3 = None
            self.stop1 = float(self.stop_inloop.text()) if len(
                self.stop_inloop.text()) != 0 else None
            self.stop2 = float(self.stop_midloop.text()) if len(
                self.stop_midloop.text()) != 0 else None
            self.stop3 = None
            self.dir1 = self.dir_inloop.currentText()
            ...
            self.X1_immune = np.linspace(self.start1, self.stop1, self.step1) if len(
                self.step_inloop.text()) != 0 else None
            self.X2_immune = np.linspace(self.start2, self.stop2, self.step2) if len(
                self.step_midloop.text()) != 0 else None
            self.X3_immune = None
            self.pointdown1 = int(self.sweepdown_inloop.text()) if len(
                self.sweepdown_inloop.text()) != 0 else None
            self.pointdown2 = int(self.sweepdown_midloop.text()) if len(
                self.sweepdown_midloop.text()) != 0 else None
            self.pointdown3 = None
            ...
            # Add Paired Instr mid
            if self.diagonalPlot_mid.isChecked() == True:
                self.instr2_paired = str(
                    self.comboPairedInstr_mid.currentText())
                self.parse_custom_mid(self.ComboFunctPair_mid.text())
                self.X2_paired = self.fonction(self.func_mid, self.X2_immune)
                print(self.X2_paired)
            else:
                self.X2_paired = np.zeros(self.step1)
            ...
            self.X1_SweepDown = np.linspace(
                self.X1[-1], self.X1[0], self.pointdown1) if len(self.sweepdown_inloop.text()) != 0 else None
            self.X2_SweepDown = np.linspace(
                self.X2[-1], self.X2[0], self.pointdown2) if len(self.sweepdown_midloop.text()) != 0 else None
            self.X3_SweepDown = None
            self.X1_plot = np.zeros((self.repeat1, self.step1))
            self.X2_plot = np.zeros((self.repeat2, self.step2))
            self.X1_plot_paired = np.zeros((self.repeat1, self.step1))
            self.X2_plot_paired = np.zeros((self.repeat2, self.step2))
            self.X3_plot = None
                    ...
                    for k in range(self.repeat1):
                        for l in range(self.step1):
                            if self.dir2 == 'Same direction':
                                ...
                                self.X2_plot_paired[i][j] = self.X2_paired[j]
                                if self.dir1 == 'Same direction':
                                    self.X1_plot[k][l] = self.X1[l]
                                    self.X1_plot_paired[k][l] = self.X1_paired[l]
                                else:
                                    if k % 2 == 0:
                                        self.X1_plot[k][l] = self.X1[l]
                                        self.X1_plot_paired[k][l] = self.X1_paired[l]
                                        ...
                                        self.X1_plot_paired[k][l] = self.X1_rev_paired[l]
                            else:
                                if i % 2 == 0:
                                    self.X2_plot[i][j] = self.X2[j]
                                    self.X2_plot_paired[i][j] = self.X2_paired[j]
                                    ...
                                    self.X2_plot_paired[i][j] = self.X2_rev_paired[j]
                                if self.dir2 == 'Same direction':
                                    self.X1_plot[k][l] = self.X1[l]
                                    self.X1_plot_paired[k][l] = self.X1_paired[l]
                                else:
...
#            self.port3 = int(self.port_outloop.text())
            ...
            self.X1_SweepDown = np.linspace(
                self.stop1, self.start1, self.pointdown1)
            self.X2_SweepDown = np.linspace(
                self.stop2, self.start2, self.pointdown2)
            self.X3_SweepDown = np.linspace(
                self.stop3, self.start3, self.pointdown3)
    ...
    def parse_custom_mid(self, string):
        if '+' in string:
            string_split = string.split('+')
            print(string_split)
            if '-' in string_split[0]:
                string_split1 = string_split[0].split('-')[1]
                char = string_split1.split('X')[0]
                self.func_mid[0] = -float(char) if char != 'X' else -1
            else:
                char = string_split[0].split('X')[0]
                self.func_mid[0] = float(char) if char != 'X' else 1
            self.func_mid[1] = float(string_split[1])
        elif '-' in string:
            string_split = string.split('-')
            if len(string_split) == 3:
                char = string_split[1].split('X')[0]
                self.func_mid[0] = -float(char) if char != 'X' else -1
                self.func_mid[1] = -float(string_split[2])
            else:
                if string_split[0] != '':
                    char = string_split[0].split('X')[0]
                    self.func_mid[0] = float(char) if char != 'X' else 1
            ...
            # for char in string:
            #    func_split.append(char)
            self.func_mid[0] = float(string.split('X')[0])
            # self.func[0] = float(func_split[0]) if func_split[0]!='X' else 1
            self.func_mid[1] = 0
