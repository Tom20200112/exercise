...
from PyQt5.QtGui import QPalette, QColor, QMenu, QToolButton
...
class Main_LoadInstr(QDialog):
    def __init__(self, parent=None):
        ...
        self.setGeometry(50,50,300,500) #Set size of GUI
        ...
        self.itctt_ch=['Probe','VTI']
        self.ipstt_ch=['Field','Temperature']
        self.sim_ch=['1','2','3','4','5','6','7','8']
        self.sr830_ch=['X','Y','R','Theta']
    ...
    def createGroup(self):
        ...
        # comment_Label = QLabel('To load an instrument, tick the corresponding checkbox and write the GPIB adress in the entry. You can see all the connected instruments and their assigned GPIB address by clicking on the \'See available resources\' button. Save your configuration by testing the connection then closing the program.')    
    ...
    def changeMenu(self, instrument, menu, idx):
        ...
        if instrument=='SR830':
            ...
            for i in self.rotator_ch:
                ...
                action.triggered.connect(lambda state, x=idx, y=ind: self.addch(x,y))
                ind+=1
        elif instrument=='iTC Teslatron':
            for i in self.itctt_ch:
                self.ch_record[idx]=[False, False]
                action = menu.addAction(i)
                action.setCheckable(True)
                action.triggered.connect(lambda state, x=idx, y=ind: self.addch(x,y))
                ind+=1
        elif instrument=='iPS Teslatron':
            for i in self.ipstt_ch:
                self.ch_record[idx]=[False, False]
                action = menu.addAction(i)
                action.setCheckable(True)
                action.triggered.connect(lambda state, x=idx, y=ind: self.addch(x,y))
